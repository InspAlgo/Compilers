...
private:
    ...
    std::map<std::set<std::string>, std::string> m_state_name; // DFA 各节点与编号名称映射

    std::string m_new_start_node;  // 生成的 DFA 的状态起始节点名称
    std::set<std::string> m_new_end_nodes;  // 生成的 DFA 的状态终止节点
    std::map<std::pair<std::string, char>, std::string> m_new_DFA;  // 生成的 DFA 状态转移表
};