...
namespace M6
{
    ...
    private:
        ...
        using Item = std::tuple<Token, std::vector<Token>>;  // 项目类型
    ...
    private:
        ...
        std::set<Token> m_nonterminals;  // 非终结符集合
        std::set<Token> m_terminals;  // 终结符集合
        std::set<Token> m_alltokens;  // 所有非终结符和终结符集合
        ...
        std::map<std::tuple<std::string, Token>, std::string> m_parsing_table;
    };
}